...
## Hidden Critical
1. #1653 - `Appender#stop` is not idempotent (Author: ppkarwasz [Member])
2. #1870 - RollingRandomAccessFile uses wrong file name inside .zip (Author: JohnLussmyer)
3. #2792 - Uniformize TLS configuration (Author: ppkarwasz [Member])
4. #2962 - Implement `MessageFactory`-namespaced logger registry (blocked by `2.24.1` release) (Author: vy [Member])
5. #3600 - [main] Add transitive `compileOnlyApi` dependencies (Author: ppkarwasz [Member])
6. #4129 - Log4J pattern layout inconsistencies when locale is specified (Author: xzel23)
